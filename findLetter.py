# The code finds the most repeated letter in a given string
...
do this. But, in a larger sense, we can not dedicate -- we can not consecrate -- we can not hallow
 ...
 the unfinished work which they who fought here have thus far so nobly advanced. It is rather for us
...
count_letters = [speech.count(letter) for letter in alphabet]
my_dict = {key:value for (key, value) in zip(alphabet, count_letters)}
maximum = max(my_dict.values())
indexOfMax = list(my_dict.values()).index(maximum)
char = list(my_dict.keys())[indexOfMax]
print(char)
